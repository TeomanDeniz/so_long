...
static inline void	check_position_monster(t_mlx mlx);
/* *************************** [^] PROTOTYPES [^] *************************** */
	...
	game(t_mlx mlx)
{
	...
	check_position_monster(mlx);
	mlx->object.player.movement_speed = \
		ft_fmax(ft_fabs(delta_x), ft_fabs(delta_y));
	mlx->steps += mlx->object.player.movement_speed;
	old_x = mlx->object.player.x;
	old_y = mlx->object.player.y;
}
...
static inline void
	check_position_monster(t_mlx mlx)
{
	register int	x;

	x = -1;
	while (++x, x < mlx->object.number_of_monsters)
	{
		if (((mlx->object.player.x <= mlx->object.monster[x].x + TILE_WIDTH && \
			mlx->object.player.x + PLAYER_WIDTH >= mlx->object.monster[x].x) \
			&& (mlx->object.player.y <= mlx->object.monster[x].y + TILE_HEIGHT \
			&& mlx->object.player.y + \
			PLAYER_HEIGHT >= mlx->object.monster[x].y)))
		{
			write(1, "\n\n YOU DIED!!!\n\n", 16);
			close_window(mlx);
		}
