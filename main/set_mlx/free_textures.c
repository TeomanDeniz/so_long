...
static inline void	free_textures2(t_mlx mlx, int ebx, int ecx);
static inline void	free_textures3(t_mlx mlx, int ebx, int ecx);
/* *************************** [^] PROTOTYPES [^] *************************** */
	...
	free_textures_directly(t_mlx mlx)
{
	...
	while (++ebx, ebx < TS_WALL)
	{
		...
		while (++ecx, !!mlx->texture_pack.wall[ebx][ecx])
			free_textures3(mlx, ebx, ecx);
	}
}

static inline void
	free_textures2(t_mlx mlx, int ebx, int ecx)
{
...
static inline void
	free_textures3(t_mlx mlx, int ebx, int ecx)
{
